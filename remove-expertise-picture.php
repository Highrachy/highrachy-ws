<?php $dashboard = true; $title = "dashboard_content"; $sub_title= "dashboard_expertise"; $sub_title2="expertise_content"; $sub_title3="remove-expertise-picture"; ?>
<?php
...
if (isset($_GET['s'])){
	...
	if ($id == ""){
		redirect("dashboard_expertise.php");
	}  else {
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST'){
			if (isset($_POST['Delete'])){
				$id = $_POST['Delete'];
				$query = "SELECT pics FROM expertise WHERE id = '$id'";
				...
				if ($total_rows == 1) { // No problems! You can delete, file exist;
					...
					$picture = $rows['pics'];
					$data['pics'] = "NULL";
					$result = $db->update_query("expertise",$data,"id=$id");
					if ($result == 1) { // If it ran OK.
						
						unlink('img/expertise/'.$picture);
						redirect('expertise_content.php?pics=remove&s='.$id);
						exit();
					}
				}
			}
                                ...
                                <img src="img/expertise/<?php echo $pics ?>"><br><br>
                                  ...
                                  <div class="align-right">
                                    <input type="submit" class="btn" value="Yes">
                              		<a href="expertise_content.php?s=<?php echo $id ?>" class="btn">No</a>
                                  </div>
