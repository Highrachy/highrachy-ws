...
public function __construct($server, $user, $pass, $database, $pre=''){
	...
	if(!@mysql_select_db($this->database, $this->link_id)) {//no database
		trigger_error("Could not open database: <b>$this->database</b>.");
		}
	
	// If no connection could be made, trigger an error:
	if (!$this->link_id) {
		trigger_error('Could not connect to MySQL: <b>$this->server</b> ');
...
// Apply trim() and mysql_real_escape_string():
return @mysql_real_escape_string(trim($data),$this->link_id);
...
protected function fetch_array($query_id=-1) {
	// retrieve row
	if ($query_id instanceof mysql_result) {
...
protected function free_result($query_id=-1) {
	...
	if((is_int($this->query_id) && ($this->query_id!=0)) && (!@mysql_free_result($this->query_id))) {
