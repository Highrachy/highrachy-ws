<?php $dashboard = true; $title = "dashboard_content"; $sub_title= "dashboard_solutions"; $sub_title2="solutions-content"; $sub_title3="edit-solutions-content"; ?>
<?php
...
if (isset($_GET['s'])){
	...
	if ($id == ""){
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST'){
			...
			$query = "SELECT name FROM solutions WHERE name='".$db->escape_data($data['name'])."' AND id <> '$id'";
			...
			$link_id = $table['link'];
				...
				redirect('solutions_content.php?s='.$link_id.'&action=update');	
		...
		$query = "SELECT name,content as editText,content, priority FROM solutions WHERE solutions.id = '$id'";
		$table = $db->fetch_first_row($query);
