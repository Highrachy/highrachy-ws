<?php $dashboard = true; $title = "dashboard_content"; $sub_title= "dashboard_career"; $sub_title2="career-content"; $sub_title3="edit-career-content"; ?>
<?php
...
if (isset($_GET['s'])){
	...
	if ($id == ""){
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST'){
			...
			//Get the parent page
			$query = "SELECT link FROM career WHERE id = '$id'";
			$table = $db->fetch_first_row($query);
			$link_id = $table['link'];
		
				redirect('career_content.php?s='.$link_id.'&action=update');	
		...
		$query = "SELECT name,content as editText,content, priority FROM career WHERE career.id = '$id'";
		$table = $db->fetch_first_row($query);
...
if (!$showInfo) {
	...
	$option = "<option>- Select career Content-</option>";
