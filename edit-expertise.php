<?php $dashboard = true; $title = "dashboard_content"; $sub_title= "dashboard_expertise"; $sub_title2="edit-expertise"; ?>
<?php
...
if (isset($_GET['s'])){
	...
	if ($id == ""){
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST'){
			...
			$query = "SELECT name FROM expertise WHERE name='".$db->escape_data($data['name'])."' AND id <> '$id' AND link=0";
			...
			if ($value >= 1) { // If it ran OK.
				redirect('dashboard_expertise.php?action=update');	
		...
		$query = "SELECT expertise.name as name, icons_id,priority,icons.name as i_name FROM expertise INNER JOIN icons ON expertise.icons_id = icons.id WHERE expertise.id = '$id'";
