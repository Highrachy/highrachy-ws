<?php $dashboard = true; $title = "dashboard_services"; $sub = "add" ?>
<?php 
...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
	...
	$query = "SELECT name FROM services WHERE name='".$db->escape_data($data['name'])."'";
	$rows = $db->total_affected_rows($query);

	if ($rows == 0) { // No problems! The services name is not in the database
			
	$value = $db->insert_query("services",$data);
	
	if ($value == 1) { // If it ran OK.
		redirect('dashboard_services.php?action=add');	
	} else { // If it did not run OK.
		trigger_error('You could not be registered due to a system error. We apologize for any inconvenience.');
	}
	
	} else { // The document name is already in the database			
			$errors[] = 'The services name you gave is already in the database';						
...
<p>We are committed to helping our clients succeed</p>  
    ...
    <div class="content" role="main">
    	...
    	<article class="post-item post-detail">
        	...
        	<h1>Add Services <p>Add a service to the Services Page</p></h1>
             <?php top_link("services"); ?>           
			<form method="post" id="custom" enctype="multipart/form-data">                         
                              ...
                              <p>
									<input type="submit" class="button_link" value="Add Services" tabindex="100">
								</p>
