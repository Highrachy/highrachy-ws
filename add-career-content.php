<?php $dashboard = true; $title = "dashboard_content"; $sub_title= "dashboard_career"; $sub_title2="add-career-content"; ?>
<?php
...
if (isset($_GET['s']))
	$id = $_GET['s'];
else redirect("career_content.php");

//Check if the id is truly a parent page.
$query = "SELECT name FROM career WHERE id='$id'";
$rows = $db->fetch_all_row($query);
if ($db->total_affected_rows() < 1)
redirect("career_content.php");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
	...
	if (empty($errors)) { // If everything's OK...
		...
		if ($value >= 1) { // If it ran OK.
			redirect('career_content.php?s='.$id.'&action=add');	
