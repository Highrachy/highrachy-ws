...
if (isset($_GET['s'])){
	...
	if ($id == ""){
			...
			if (empty($errors)) { // If everything's OK...
				...
				if ($rows <= 1) { // No problems! The product name is not in the database
					...
					if($change_pics){
						...
						$image_mid->render();
						$data['pics'] = $fileName;
						unlink('img/expertise/'.$old_picture);
					}
		...
		$query = "SELECT  expertise.id,name, home.pics FROM expertise INNER JOIN home ON expertise.id = home.link  WHERE expertise.id = '$id'";
