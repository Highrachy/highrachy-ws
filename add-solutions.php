<?php $dashboard = true; $title = "solutions"; $sub_title= "add-solutions"; $script=true; ?>
<?php
...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
    ...
    // Check if the solutions name is in the database
	$query = "SELECT name FROM solutions WHERE name='".$db->escape_data($data['name'])."' AND link=0";
	$rows = $db->total_affected_rows($query);

	if ($rows == 0) { // No problems! The solutions name is not in the database
			
	$value = $db->insert_query("solutions",$data);
	
	if ($value >= 1) { // If it ran OK.
		redirect('dashboard_solutions.php?action=add');	
                    ...
                    <div id="tab-one">
                      ...
                      <div class="list-wrap">
                          ...
                          <form method="post" id="custom" enctype="multipart/form-data">                         
                                 ...
                                 <div class="clearfix"><small><a href="all-icons.php" class="icons align-right pad30" target="_blank">View All Icons</a></small></div>
